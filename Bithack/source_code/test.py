...
def wait_for_test_process(proc, timeout):
    ...
    while proc.returncode is None and time.time() < endtime:
        time.sleep(0.1)
        err_chunks.append(proc.stderr.read(4096))
        proc.poll()
...
def main(argv):
    ...
    print('Ran %d test functions, %d individual tests passed, %d individual tests failed.' % (total_tests-1, total_passes, total_failed))
